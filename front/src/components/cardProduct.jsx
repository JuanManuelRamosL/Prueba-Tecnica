...
const ProductCard = ({ product }) => {
  const { addToCart, fetchProductById } = useProductStore();
  ...
  const handleProductClick = async () => {
    ...
    router.push(`/product/${product.id}`); // Redirige a la página de detalle
  };

  return (
    <div
      onClick={handleProductClick}
      className="cursor-pointer w-full max-w-lg p-4 border border-gray-300 rounded-lg mb-5"
    >
